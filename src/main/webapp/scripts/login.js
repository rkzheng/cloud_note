...
function registUser(){
	...
	if(name==""){
		ok = false;
		$("#warning_1").show();
		$("#warning_1 span").html("Username is empty");
	}
	if(password==""){
		ok = false;
		$("#warning_2").show();
		$("#warning_2 span").html("Password is empty");
	}else if(password.length<6){
		ok = false;
		$("#warning_2").show();
		$("#warning_2 span").html("Password is too short");
	}
	if(f_password==""){
		ok = false;
		$("#warning_3").show();
		$("#warning_3 span").html("Confirm password is empty");
	}else if(f_password!=password){
		ok = false;
		$("#warning_3").show();
		$("#warning_3 span").html("Password does not match");
	}
	//发送Ajax请求
	if(ok){
		$.ajax({
...
function checkLogin(){
	...
	if(name==""){
		ok = false;
		$("#count_span").html("Username is empty");
	}
	if(password==""){
		ok = false;
		$("#password_span").html("Password is empty");
	}
	//发送Ajax请求
	if(ok){
		$.ajax({
			...
			success:function(result){
				...
				if(result.status==0){//成功
					...
					//写入Cookie
					addCookie("uid",user.cn_user_id,24);
					addCookie("uname",user.cn_user_name,24);
